...
my ($ko, $koMap, $diff, $outdir, $help);

GetOptions("ko:s" => \$ko, "komap:s" => \$koMap, "diff:s" => \$diff, "outdir:s" => \$outdir, "help|?" => \$help);
$outdir ||= ".";
my $mapDir = dirname($koMap) . "/map";
do "$Bin/keggConf.pl";
...
if (!defined $ko || !defined $diff || !defined $koMap || defined $help) {
	&usage();
	exit 1;
}
...
=cut
					if (/rect\s[^\d]*([\d]+),([\d]+)[^\d]*([\d]+),([\d]+).*$ko(\+.*)*/) {
						...
						$htmlData{"map$mapId"}{"$1,$2,$3,$4"}{$color}{$ko}{$temp[0]} = $ratios{$temp[0]};
					}
					elsif (/poly\s[^\d]*([\d]+),([\d]+)[^\d]*([\d]+),([\d]+),([\d]+),([\d]+),([\d]+),([\d]+).*$ko(\+.*)*/){
						push @{$data{$mapImg}}, [$temp[0], $1, $2, $3, $4, $5, $6, $7, $8, $color, "poly"];
						$htmlData{"map$mapId"}{"$1,$2,$3,$4,$5,$6,$7,$8"}{$color}{$ko}{$temp[0]} = $ratios{$temp[0]};
					}
					elsif (/poly\s[^\d]*([\d]+),([\d]+)[^\d]*([\d]+),([\d]+),([\d]+),([\d]+).*$ko(\+.*)*/){
						push @{$data{$mapImg}}, [$temp[0], $1, $2, $3, $4, $5, $6, $color, "poly"];
						$htmlData{"map$mapId"}{"$1,$2,$3,$4,$5,$6"}{$color}{$ko}{$temp[0]} = $ratios{$temp[0]};
					}
				}
				close CONF;
			}
