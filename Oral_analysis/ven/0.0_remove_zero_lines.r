...
#check3 = apply(profile[,HD_sample_number], 1, save_or_rm, cut = 18)
#check4 = apply(profile[,LD_sample_number], 1, save_or_rm, cut = 23)
profile_final1=profile[check1,]
profile_final2=profile[check2,]
name1<-rownames(profile_final1)
name2<-rownames(profile_final2)
...
write.table(name1, paste( args[2],"list",sep="."), quote=F,sep="\t")
write.table(name2, paste( args[3],"list",sep="."), quote=F,sep="\t")
