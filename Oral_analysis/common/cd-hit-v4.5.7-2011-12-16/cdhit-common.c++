...
void Options::Validate()
{
	...
	if( useDistance ){
		...
		if ((cluster_thd > 1.0) || (cluster_thd < 0.75)) bomb_error("invalid clstr threshold, should >=0.75");
	}else{
...
void SequenceDB::Read( const char *file, const Options & options )
{
	...
	if( options.not_store ) {}
    else if( options.store_disk ) swap = OpenTempFile( temp_dir );
	Clear();
	dummy.swap = swap;
...
void SequenceDB::DivideSave( const char *db, const char *newdb, int n, const Options & options )
{
	...
	for (i=0; i<n; i++){
		...
		if( seg_size >= max_seg ){
			fclose( fout );
			//strcat( outfile, ".done");   //add by chenwn to mark divide one file success;
			//fout = fopen( outfile, "w"); //add by chenwn
			//fclose( fout );              //add by chenwn
			seg += 1;
