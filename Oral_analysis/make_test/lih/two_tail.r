...
q_values=p.adjust(p_values,method=adjust_method)

reduced_profile=profile_table[which(q_values< cutoff),]
...
#den=reduced_profile[which(reduced_vector_1 == reduced_vector_2),]
reduced_profile_2=reduced_profile[which(reduced_vector_1 < reduced_vector_2),]



write.table(reduced_profile_1, paste(args[4], ".list",sep=""), col.names=T,sep="\t", quote=F)
write.table(reduced_profile_2, paste(args[5], ".list",sep=""), col.names=T,sep="\t", quote=F)
#write.table(den,"Den",col.names=T,sep="\t",quote=F)
