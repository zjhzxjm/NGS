...
my ($Global,$Node, $Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,$Job_prefix,$Sub_Prjct_Id,$Verbose, $Help, $getmem);
GetOptions(
	"global"=>\$Global,
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
...
#	$job_cmd .= "-q $Queue "  if(defined $Queue); ##set queue
	if(defined $Queue){				##set queue
		$job_cmd .= "-q $Queue ";
#		if($Queue eq "bc.q"){
#			$job_cmd .= "-P bc_gag ";
#		}
#		if($Queue eq "supermem.q"){
#			$job_cmd .= "-P supermem ";
#		}
	}
	if(defined $Sub_Prjct_Id){
		$job_cmd .= " -P $Sub_Prjct_Id ";
	}
...
#xujm
#my $gather="/ifshk1/BC_gag/PROJECT/EPI/tangqiqin/RNA/job.mem.info/";
#if (-d "/ifs2/BC_GAG/")
#{
#	$gather="/ifs2/BC_GAG/Project/RNA_denovo/xutong/software/RNAdenovo.job.log";
#}
...
system "echo qsub-sge.pl finished |mail -s '$work_shell_file finished' xujm\@realbio.cn";
